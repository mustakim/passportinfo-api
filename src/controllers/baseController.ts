...
export const BaseController: any = (
  ...
  controllerName = controllerName.split('Controller')[0];
  ...
  const index = async (req: any, res: any) => {
    try {
      ...
      if (option.index && option.index.order) {
        order = option.index.order;
      } else {
        order = [['id', 'DESC']];
      }
  ...
  const info = async (req: any, res: any) => {
    try {
      res.status(200).json({
        success: true,
        message: `from  iformation  of ${controllerName} `,
        data: Object.keys(Model.rawAttributes)
      });
    } catch (err) {
      res.status(400).json({
        success: false,
